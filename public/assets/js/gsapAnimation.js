(function ($) {
    ...
    var scrollEffectFade = () => {
        if ($(".effectFade").length) {
            ...
            document.querySelectorAll(".effectFade").forEach((el) => {
                ...
                // If the element is already inside the viewport when the
                // animations initialise (common in Next.js where scripts load
                // after React hydration), ScrollTrigger's deferred position
                // evaluation can silently miss the trigger.  Animate directly
    ...
    var loader = function () {
        if ($(".preloader").length) {
            ...
            // No preloader: runAnimations() is called externally by ScriptSequencer
            // after trigger('load') so Swiper is already initialised.
        }
    };
    ...
    window.scrollEffectFade = scrollEffectFade;
